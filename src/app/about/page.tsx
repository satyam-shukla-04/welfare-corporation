...
import { PageHero, PremiumCTA, SectionHeader, reveal } from "@/components/PremiumUI";
        ...
        <div className="container mx-auto px-6">
          <div className="grid gap-10 lg:grid-cols-2 lg:items-center">
            <motion.div
              ...
              whileInView="visible"
              viewport={{ once: true }}
              className="relative aspect-square overflow-hidden rounded-[2.5rem] shadow-premium"
            >
              <Image
              ...
              <div className="mt-8 grid grid-cols-2 gap-4">
                {[
                  ...
                  <div key={label} className="rounded-[2rem] border border-slate-200 bg-white/80 p-6 shadow-premium backdrop-blur-2xl">
                    ...
                    <div className="mt-2 text-xs font-bold uppercase tracking-[0.2em] text-slate-400">{label}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            {[
              ...
              <motion.div
                key={item.title}
                whileHover={{ y: -8 }}
                className={index === 1 ? "rounded-[2rem] bg-primary p-8 text-white shadow-glow" : "rounded-[2rem] border border-slate-200 bg-white p-8 shadow-premium"}
              >
                <item.icon className={index === 1 ? "mb-10 text-accent" : "mb-10 text-accent"} size={34} />
                <h3 className="text-3xl font-black">{item.title}</h3>
                <p className={index === 1 ? "mt-5 leading-8 text-white/65" : "mt-5 leading-8 text-slate-600"}>{item.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
