          ...
          {SERVICES.map((service, index) => (
            ...
            >
              ...
              <div className="max-w-xl">
                ...
                <h2 className="mt-5 text-4xl font-black tracking-tight text-text-primary md:text-5xl">{service.title}</h2>
                <p className="mt-6 text-lg leading-8 text-text-secondary">{service.description}</p>
            ...
            <div className="mt-8 grid gap-4">
              {["Quick Response Time", "Tailored Pricing Models", "Corporate Account Benefits"].map((item) => (
                <div key={item} className="flex items-center gap-4 rounded-[1.5rem] border border-slate-200 bg-white/80 p-4 shadow-premium">
                  ...
                  <span className="font-black text-text-primary">{item}</span>
                </div>
              ))}
            </div>
          </div>
